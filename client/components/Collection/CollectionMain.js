		...
		<section className="flex-80 px-8 bg-gray-200">
			<header className="flex justify-between align-center py-4 w-full">
				...
				<nav>
					<ul className="flex items-center">
						<li className="ml-4">
							<Link to="/admin/products/collections/new" className="bg-green-500 px-4 py-2 text-white rounded inline-block	">
								Add Collection
							</Link>
						</li>
					</ul>
			...
			<main>
				...
				{collection.allCollection?.map((c) => {
					return <SingleCollection collection={c} state={state} setState={setState} />;
				})}
			</main>
		</section>
  );
