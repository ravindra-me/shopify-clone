  ...
  return (
		<section className="flex-80 px-8 bg-gray-200">
			<header className="flex justify-between align-center py-4 w-full">
				<Link to="/admin/products/collections" className="px-4 py-1 bg-gray-300 rounded text-xl">
					...
					<div className="flex-50">
						<div className="bg-white p-8 rounded">
							<div>
								...
								<input className="block border w-full p-1 mt-1" type="text" value={title} id="title" name="title" onChange={handleChange} />
								<div className="mt-8">
									<label for="description">Description</label>
									<textarea className="block border w-full p-1 mt-1" type="text" value={description} id="description" onChange={handleChange} name="description"></textarea>
								</div>
							</div>
						</div>
						<div className="bg-white p-8 mt-4">
							...
							<div>
								...
								<p className="ml-8">Add products to this collection one by one. Learn more about manual collections.</p>
							</div>
							<div className="mt-4">
								<div className="mb-2">
									<input
								...
								<p className="ml-8">Existing and future products that match the conditions you set will automatically be added to this collection. Learn more about automated collections.</p>
							</div>
						</div>
						{isAutomated && <Condition collection={collection} setCollection={setCollection} />}
					</div>
					<aside className="flex-30">
						<div className="bg-white p-8">
							<div className="mb-4">
								<h4 className="font-bold">Collection availability</h4>
								<p className="mt-2 text-gray-500">Will be available to 1 sales channel.</p>
							</div>
							<div>
								<div>
									<input
										...
										checked={onlineStore}
										onChange={(event) => {
                      setCollection({ ...collection, onlineStore : !collection.onlineStore});
										}}
									/>
						...
						<div className="mt-4 bg-white p-8">
							...
							<div className="text-center my-4 border p-4 rounded bg-blue-200">
								<label for="image">
									<img width="40" src="data:image/svg+xml,%3csvg fill='none' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 20 20'%3e%3cpath fill-rule='evenodd' clip-rule='evenodd' d='M20 10a10 10 0 11-20 0 10 10 0 0120 0zM5.3 8.3l4-4a1 1 0 011.4 0l4 4a1 1 0 01-1.4 1.4L11 7.4V15a1 1 0 11-2 0V7.4L6.7 9.7a1 1 0 01-1.4-1.4z' fill='%235C5F62'/%3e%3c/svg%3e" name="image" className="mx-auto" onChange={handleChange}></img>
								</label>
								<input type="file" id="image" className="mt-4" name="image" onChange={handleChange}></input>
							</div>
						</div>
					</aside>
				</section>
