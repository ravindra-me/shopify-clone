...
function Signup(props) {
	...
	const handleChange = (event) => {
		...
		switch (name) {
			case "firstName":
				errors.firstName = value.length >= 3 ? "" : "it must be include 3 latter";
				break;
			case "lastName":
				errors.lastName = value.length >= 3 ? "" : "it must be include 3 latter";
				break;
			case "email":
				errors.email = value.indexOf("@") === -1 ? "email must be include @ " : "";
				break;
			case "password":
				let passwordError;
	...
	return (
		<form className="flex-40 shadow-lg shadow border border-indigo-600 bg-gray-100 rounded-lg ">
			...
			<div className="mb-4">
				<input type="text" name="firstName" placeholder="First Name" className="bg-white" value={firstName} onChange={handleChange} />
			...
			<div className="mb-4">
				<input type="number" name="phone" placeholder="contact number" value={phone} onChange={handleChange} />
				<span className="text-red-500">{errors.phone}</span>
			</div>

      <button className="hover:bg-black rounded" onClick={(e) => {
        e.preventDefault()
        props.dispatch(
			newUser({
